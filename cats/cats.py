...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    result_list = [ s for s in paragraphs if select(s)]
    ...
    if typed_words == [] and source_words == []:
        ...
        for i in range(len(typed_words)):
            ...
            if typed_words[i] == source_words[i]:
                correctNum += 1
        return correctNum / len(typed_words) * 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list: 
        diff.setdefault(word,diff_function(typed_word, word, limit))
    
    min_value = min(diff.values(), key=abs)
    if min_value > limit:
        return typed_word
    else:
        for key in diff:
            if min_value == diff[key]:
                return key
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    def helper(t, s, cnt):
        ...
        if t[0] != s[0]:
            cnt += 1
        return helper(t[1:], s[1:], cnt)
    return helper(typed,source,abs(len(source) - len(typed)))
    # END PROBLEM 6
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    words = [[] for _ in range(word_indices)]
    for word_index in range(word_indices):
        fastest_player = 0
        for player_num in range(player_indices):
            if time(match, player_num, word_index) < time(match, fastest_player, word_index):
                fastest_player = player_num
        words[fastest_player].append(get_word(match,word_index))
    return words
    # END PROBLEM 10
