...
class ThrowerAnt(Ant):
    ...
    lower_bound = -1 #LongThrower
    upper_bound = float('inf') #ShortThrower
    ...
    def nearest_bee(self):
        ...
        while not pos.is_hive:
            if len(pos.bees) > 0 and step <= self.upper_bound and self.lower_bound <= step:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while pos.exit is not None:
            if pos.ant:
                if not pos.ant.is_doubled and pos.ant is not FireAnt:
                    pos.ant.double()
                    pos.ant.is_doubled = True
                if pos.ant.is_container and pos.ant.ant_contained and pos.ant.ant_contained is not FireAnt and not pos.ant.ant_contained.is_doubled:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not pos.is_hive:
            ...
            if pos.ant:
                # 非容器
                if not pos.ant.is_container and pos.ant is not self:
                    InsectDisc[pos.ant] = d
                # 容器
                if pos.ant.is_container:
                    InsectDisc[pos.ant] = d
                    if pos.ant_contained is not self:
                        InsectDisc[pos.ant_contained] = d
            pos = pos.entrance
            d += 1
        return InsectDisc
        # END Problem Optional 2
